...
## ./make_putcorn.sh [url]
##
## The script make_putcorn.sh allows you to clone, setup, and build a version of putcorn time
...
## ./make_putcorn.sh '-b release/0.3.4 https://github.com/barisariburnu/putio-popcorn-time.git'
##


clone_repo="True"
if [ -z "${1}" ]; then
    clone_url="https://github.com/barisariburnu/putio-popcorn-time.git"
elif [ "${1}" = "ssh" ]; then
    clone_url="ssh://git@github.com:barisariburnu/putio-popcorn-time.git"
else
    clone_url="${1}"
fi
...
clone_command() {
    if git clone ${clone_url} ${dir}; then
        echo "Cloned Putcorn Time successfully"
    else
        echo "Putcorn Time encountered an error and could not be cloned"
        exit 2
    fi
}
...
if [ -e ".git/config" ]; then
    ...
    case ${dat} in *putcorn-app*)
        echo "You appear to be inside of a Putcorn Time repository already, not cloning"
        clone_repo="False"
        ;;
    *)
        try="True"
...
if [ "${clone_repo}" = "True" ]; then
    echo "Cloning Putcorn Time"
    read -p "Where do you wish to clone putcorn time to? [putcorn-app] " dir
    if [ -z "${dir}" ]; then
        dir='putcorn-app'
    elif [ "${dir}" = "/" ]; then
        dir='putcorn-app'
...
tries=0
while [ "${try}" = "True" ]; do
    read -p "Do you wish to install the required dependencies for Putcorn Time and setup for building? (yes/no) [yes] " rd_dep
    if [ -z "${rd_dep}" ]; then
        rd_dep="yes"
    fi
    tries=$((${tries}+1))
...
if [ "${rd_dep}" = "yes" ]; then
    ...
    echo "Successfully setup for Putcorn Time"
fi

if grunt build; then
    echo "Putcorn Time built successfully!"
    echo "Run 'grunt start' from inside the repository to launch the app"
    echo "Enjoy!"
else
    echo "Putcorn Time encountered an error and couldn't be built"
    exit 5
fi
